...
public class ParticleMenu : MonoBehaviour {
	...
	public void Navigate(int i){
		...
		_currentIndex = (particleSystems.Length + _currentIndex + i) % particleSystems.Length;
		...
		_currentGo = Instantiate (particleSystems[_currentIndex].particleSystemGo, spawnLocation.position + particleSystems[_currentIndex].particlePosition, Quaternion.Euler(particleSystems[_currentIndex].particleRotation)) as GameObject;

		// only activate the gun GameObject if the current effect is a weapon effect
		gunGameObject.SetActive (particleSystems[_currentIndex].isWeaponEffect);

		// setup the UI texts according to the strings in the array 
		title.text = particleSystems [_currentIndex].title;
		description.text = particleSystems [_currentIndex].description;
		navigationDetails.text = $"{(_currentIndex + 1)} out of {particleSystems.Length}";
