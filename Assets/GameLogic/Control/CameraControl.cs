    ...
    public class CameraControl : MonoBehaviour
    {
        ...
        public float
            cameraSpeedModifierMultiplier =
                2.5f; // global camera speed sensitivity multipler when a special modifer key is held down

        #region MouseControlConfiguration

        // camera scrolling sensitivity
        [Header("Scrolling")] public float scrollingSensitivityModifier = 10f;

        // edge scrolling
        [Header("Edge scrolling")] public bool allowEdgeScrolling = false;
        public int edgeScrollDetectBorderThickness = 15;

        // mouse control camera translation
        [Header("Mouse Scrolling")] public bool allowMouseTranslation = true;
        public float mouseTranslationSensitivityModifier = 0.75f; // mouse translation movement speed modifier

        // mouse rotation control
        [Header("Mouse Rotation")] public bool allowMouseRotation = true;
        public float mouseRotationSensitivityModifier = 50f; // mouse rotation movement speed modifier

        // zoom with FOV
        [Header("Camera zoom")] public bool allowCameraZoom = true;
        ...
        [Header("Camera movement inertia")] public bool allowCameraInertia = true;
        [Range(0.01f, 0.99f)] public float inertiaDecay = 0.95f;

        // camera restriction
        [Header("Camera restriction")] public float cameraVerticalAngleMin = 10f;
        public float cameraVerticalAngleMax = 80f;

        public float
            viewCenterOffset = 200f; // camera view center point offset; calculated as this far in front of camera

        public float
            viewCenterOnPlayerOffset =
                75f; // how far from player position the camera will be set when focusing on player
        ...
        public float
            minCameraToGroundDistance = 2f; // how close to ground the camera can go before limiter will start resisting
        ...
        public float
            cameraTooLowSpeedLimiter = 5f; // this one needs to be resistive otherwise camera will dip into objects
        ...
        private void RestrictCamera()
        {
            ...
            try
            {
                ...
                if (offsetAboveFloor < 0)
                {
                    ...
                    transform.position -=
                        new Vector3(0, offsetAboveFloor, 0) * (limiterInertia * cameraTooLowSpeedLimiter);
                }
                else if (offsetBelowCeiling < 0)
                {
                    // camera too high
                    transform.position += new Vector3(0, offsetBelowCeiling, 0) *
                                          (limiterInertia * cameraTooHighSpeedLimiter);
                }
            }
        ...
        {
            return /*GameControl.gameSession.humanPlayer.getPos()*/
                new Vector3(0, 0, 0) - transform.forward * viewCenterOnPlayerOffset;
        }
    }
}
