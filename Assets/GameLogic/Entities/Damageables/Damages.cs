...
namespace Entities.Damageables
{
    ...
    public struct Damage : ICloneable<Damage>
    {
        ...
        // medium penetration, medium dispersion
        public static Damage SlashingDamage(float amount) => new Damage(EDamageType.Slashing, amount, 0.2f, 0.3f);

        // high penetration, low dispersion
        public static Damage PiercingDamage(float amount) => new Damage(EDamageType.Piercing, amount, 0.5f, 0.05f);

        // very low penetration, medium dispersion
        public static Damage BluntDamage(float amount) => new Damage(EDamageType.Blunt, amount, 0.05f, 0.25f);

        // very low penetration, very high dispersion
        public static Damage ChemicalDamage(float amount) => new Damage(EDamageType.Chemical, amount, 0.05f, 1f);

        // very high penetration, very high dispersion
        public static Damage ElectricDamage(float amount) => new Damage(EDamageType.Electric, amount, 1f, 1f);

        public static Damage RandomDamage(float amount)
        {
            // lambda expressions to not initialize damages
            var damages = new List<Func<float, Damage>>()
            {
                SlashingDamage,
                PiercingDamage,
                BluntDamage,
            ...
            // get a random index for the above lists
            float r = UnityEngine.Random.value;
            int index = Mathf.RoundToInt(r * damages.Count);

            // construct the damage
            return damages[index](amount);
        }
        ...
        public static List<EDamageType> DamageTypes
        {
            get
            {
                ...
                if (_damageTypes is null)
                {
                    ...
                    foreach (EDamageType damageType in Enum.GetValues(typeof(EDamageType)))
                    {
